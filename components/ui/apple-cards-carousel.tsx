...
import { SpatialContainer, usePinch } from "@webspatial/react-sdk";
...
export const Card = ({ card, index, layout = false }: { card: Card; index: number; layout?: boolean }) => {
  ...
  // Animate popout with Z and scale
  return (
    <SpatialContainer>
      <>
        <AnimatePresence>
          {(open || isPinched) && (
            <div className="fixed inset-0 h-screen z-50 overflow-auto">
              ...
              <motion.div
                ...
                animate={{ opacity: 1, scale: 1.1, z: 200 }}
                exit={{ opacity: 0, scale: 0.8, z: 0 }}
                transition={{ type: "spring", stiffness: 300, damping: 30 }}
                ref={containerRef}
                layoutId={layout ? `card-${card.title}` : undefined}
                className="max-w-5xl mx-auto bg-white dark:bg-neutral-900 h-fit z-[60] my-10 p-4 md:p-10 rounded-3xl font-sans relative"
                style={{ boxShadow: "0 8px 32px rgba(0,0,0,0.25)", transform: "translateZ(200px) scale(1.1)" }}
              >
  ...
  export const BlurImage = ({ height, width, src, className, alt, ...rest }: ImageProps) => {
    ...
    return (
      <Image className={cn("transition duration-300", isLoading ? "blur-sm" : "blur-0", className)} onLoad={() => setLoading(false)} src={src} width={width} height={height} loading="lazy" decoding="async" blurDataURL={typeof src === "string" ? src : undefined} alt={alt ? alt : "Background of a beautiful view"} {...rest} />
    );
  };
// ...existing code...
